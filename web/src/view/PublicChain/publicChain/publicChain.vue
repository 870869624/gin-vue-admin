  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="日期" prop="createdAt" width="180">
          ...
          <el-table-column align="left" label="公链名称" prop="name" width="120" />
          <el-table-column label="公链图片" prop="picture" width="200">
              <template #default="scope">
                <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.picture)" fit="cover"/>
              </template>
          </el-table-column>
          <el-table-column label="公链logo" prop="logo" width="200">
              <template #default="scope">
                <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.logo)" fit="cover"/>
              </template>
          </el-table-column>
          <el-table-column align="left" label="公链简介" prop="introduction" width="120" />
          <el-table-column align="left" label="公链网址" prop="url" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updatePublicChainFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="公链名称">
                        {{ detailFrom.name }}
                    </el-descriptions-item>
                    <el-descriptions-item label="公链图片">
...
const getTableData = async() => {
  const table = await getPublicChainList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deletePublicChainByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePublicChainFunc = async(row) => {
    const res = await findPublicChain({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePublicChainFunc = async (row) => {
    const res = await deletePublicChain({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
